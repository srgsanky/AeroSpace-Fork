...
aerospace list-windows [-h|--help] --all [--stashed (yes|no)] [--format <output-format>] [--count] [--json]
...
--stashed (yes|no):: Filter results by stash membership.
...
%{window-parent-container-layout}:: String. The layout (`v_tiles`, `h_tiles`, `v_accordion`, `h_accordion`, `floating`, `stashed`) of the window's parent container.
%{window-state}:: String. One of `tiled`, `floating`, `accented`, `stashed`, `macos_native_minimized`, `macos_native_fullscreen`, `macos_native_window_of_hidden_app`, or `popup`.
